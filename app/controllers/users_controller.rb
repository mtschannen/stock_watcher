class UsersController < ApplicationController
  def after_ch_auth 
    ...
    # @res = HTTP.post("https://sandboxcernerhealth.com/oauth/access_token", :form => {:client_id => "", :redirect_uri => 'http://obduros.com:3000', :client_secret => 'cb5tG9Ez5jW8k62nN3Sa92u8gn5Ba3dR', :code => @code })
  ...
  def before_ch_auth
    ...
    # redirect_to "https://sandboxcernerhealth.com/oauth/authenticate?client_id=&redirect_uri="
  ...
  def before_ch_out
    ...
    # @res = HTTP.post("https://sandboxcernerhealth.com/oauth/logout_nonce", :form => {:client_id => "", :redirect_uri => 'http://obduros.com:3000/logout', :client_secret => 'cb5tG9Ez5jW8k62nN3Sa92u8gn5Ba3dR' })
    # @data = JSON.parse(@res, symbolize_keys: true)
    #gets info sent back by ch, then confirms logout by sending a second request paired with 'nonce' key
    # redirect_to ("https://sandboxcernerhealth.com/oauth/logout?client_id=&redirect_uri=&nonce=" + @data['nonce'])
